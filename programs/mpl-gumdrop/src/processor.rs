...
use crate::error::MplGumdropError;
use crate::instruction::accounts::CreateAccounts;
use crate::instruction::{CreateArgs, MplGumdropInstruction};
...
pub fn process_instruction<'a>(
    ...
    let instruction: MplGumdropInstruction =
        MplGumdropInstruction::try_from_slice(instruction_data)?;
    match instruction {
        MplGumdropInstruction::Create(args) => {
...
fn create<'a>(accounts: &'a [AccountInfo<'a>], args: CreateArgs) -> ProgramResult {
    ...
    if *ctx.accounts.system_program.key != system_program::id() {
        return Err(MplGumdropError::InvalidSystemProgram.into());
    }
